...
public class UserServiceImpl implements UserService{
    ...
    public UserModel getUserByIdInCache(Integer id) {
        UserModel userModel = (UserModel) redisServive.getKey("user_validate_"+id);
        // 如果Redis中不存在就从数据库中取，并存入缓存中
        if (userModel == null){
            userModel = this.getUserById(id);
            redisServive.setKey("user_validate_"+id,userModel);
            redisServive.expire("user_validate_"+id,10, TimeUnit.MINUTES);
        }
        return userModel;
    }
